(function () {
    ...
    WinJS.Namespace.define("server", {
        ...
        base: function (the_sel_age) {
            ...
            } else if (the_sel_age === "Function A") {
                var query = Age.where({
                    AccessS: true,
                    Access: 1
                }).read().done(function (results) {
        ...
        protein: function (){
           ...
           var roamingSettings = appData.roamingSettings;
            if (roamingSettings.values["Age_name"] === "Toddler") {
                var query = Age.where({
                    Access: 3,
                    AccessT: true
                }).read().done(function (results) {
            ...
            } else if (roamingSettings.values["Age_name"] === "Youth") {
                var query = Age.where({
                    Access: 3,
                    AccessY: true
                }).read().done(function (results) {
            ...
            } else if (roamingSettings.values["Age_name"] === "Teen") {
                var query = Age.where({
                    Access: 3,
                    AccessTT: true
                }).read().done(function (results) {
            ...
            } else if (roamingSettings.values["Age_name"] === "Adult") {
                var query = Age.where({
                    Access: 3,
                    AccessA: true
                }).read().done(function (results) {
            ...
            } else if(roamingSettings.values["Age_name"] === "Senior"){
                var query = Age.where({
                    Access: 3,
                    AccessS: true
                }).read().done(function (results) {
